...
class SampleParams {
	...
	std::string m_indicatorID;
	std::string m_indicatorsPath;

public:

	std::string getIndicatorID() const
	{
	...
	static void printHelp(const std::string& procName) {
		...
		std::cout << "/indicatorid | --indicatorid | /i | -i" << std::endl;
		std::cout << "Indicator ID. Parameter is optional.\n" << std::endl;
	}
	...
	void printSampleParams(std::string procName) const
	{
		...
		std::cout << "Indicators_dir_path='" << getIndicatorsPath() << "'\nIndicatorID='" << getIndicatorID() << "'" << std::endl;
	}
	...
	SampleParams(int argc, char **argv) {
		...
		m_indicatorID = getArgument(argc, argv, "i");
		...
		if (m_indicatorID.empty())
			m_indicatorID = getArgument(argc, argv, "indicatorid");
	}
