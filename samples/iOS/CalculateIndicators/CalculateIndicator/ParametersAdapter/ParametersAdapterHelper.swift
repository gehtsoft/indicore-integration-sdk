...
{
    static let dateFormatterString = "yyyy.MM.dd"
    ...
    private static func parseDate(oleDate: Double, isNil: Bool) -> String
    {
        ...
        if (oleDate < 0)
        {
            ...
            let currentDate = Date()
            let localDateWithOffset = calendar.date(byAdding: offset, to: currentDate)
            let formatter = DateFormatter()
            formatter.timeZone = TimeZone(abbreviation: "UTC")
            formatter.dateFormat = dateFormatterString
            let res = formatter.string(from: localDateWithOffset!)
            return res
        }
